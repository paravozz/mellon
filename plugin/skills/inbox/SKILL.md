...
1. Call the `mellon` MCP tool `check_inbox`. If it's empty, say so in one line and stop.
...
3. For each message:
   ...
   - Send each answer with `reply` and the message's `thread_id`, and show the user what you sent (gist is fine if the full reply is long).
