...
- ON YOUR FIRST TURN: arm the watcher — run sh "${SCRIPT_DIR}/watch.sh" as a background task (run_in_background, generous timeout). It long-polls the bridge and exits the moment a teammate's question arrives, or after ~60 min of quiet. Whenever it exits — for any reason (message reported, quiet, task timeout) — do what its output says, then immediately re-arm it the same way. It refuses to double-arm, so re-arming is always safe. This is what makes delivery instant even while the session is idle.
