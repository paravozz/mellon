...
# message routed to THIS session arrives. Run it as a harness background task —
# its exit notification is what re-invokes the agent instantly, even in an idle
# session. Each arm lasts ~60 min of quiet (or until a message); the agent
# re-arms it on every exit.
#
# Pure POSIX shell + curl; mail detection is a substring check, so it works on
# machines with no Python or Node at all.
#
# One watcher per (agent, broker, session) — atomic mkdir lock, matching
# presence.sh. SessionEnd kills it; an orphan guard makes it exit if the
# session that armed it dies uncleanly. Always exits 0; the final stdout line
# says what to do next.
#
# Env: MELLON_URL / MELLON_TOKEN / MELLON_AGENT_ID (config),
#      MELLON_SESSION (this session's key, from the session instructions),
#      MELLON_WAIT_SECS (default 50), MELLON_WATCH_CYCLES (default 72 ≈ 60 min;
#      0 = run for the whole session).
...
# mkdir is atomic: exactly one watcher per (agent, broker, session), no races.
...
while [ "$CYCLES" -eq 0 ] || [ "$i" -lt "$CYCLES" ]; do
  ...
  RESP="$(curl -fsS --max-time $((WAIT_SECS + 10)) \
    "$MELLON_URL/wait?agent_id=$MELLON_AGENT_ID&timeout=$WAIT_SECS&session_key=$SESSION_KEY" \
  ...
  case "$RESP" in
    *'"thread_id"'*)
      ...
      FROM="$(printf '%s' "$RESP" | tr '{,' '\n\n' \
        ...
        | sort -u | tr -d '"\\' | tr '\n' '~' | sed 's/~$//; s/~/, /g')"
      echo "mellon-watch: ${N} unread message(s) for this session from ${FROM:-teammate agents}. Show the question(s) verbatim to the user. Mid-task: ask whether to answer now or continue. Idle: answer via mellon check_inbox (pass your session key) from the actual code of this repo. Then re-arm the watcher."
      exit 0
      ;;
  esac
done
